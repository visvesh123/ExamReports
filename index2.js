...
<style>
  ...
  .branch-title { margin-top:20px; font-weight:bold; font-size:14px; }
...
<body>
  ...
  {{#each branches}}
    ...
    <div class="footer">
      <p>Total Present: <b>{{present_count}}</b></p>
      <p>Total Absentees: <b>{{absent_count}}</b></p>
...
//   console.log(attendanceRows)
  ...
  if (studentIds.length) {
    const { data, error } = await supabase
      .from("students")
      .select("student_id, ht_no, full_name , branch")
    ...
    console.log(exams)
     const { data: courses } = await supabase
      ...
      for (const exam of exams) {
        ...
        // First attach branch to each student
let students = attendanceRows.map((row, index) => {
    const s = studentById[row.student_id] || {};
    ...
    return {
      ...
      ht_no: s.ht_no || "",
      full_name: s.full_name || "",
      branch: s.branch || "UNKNOWN",   // 👈 important
      status: row.status || ""
    };
  });
  ...
  const branchTables = Object.keys(groupedByBranch).map(branch => ({
    branch,
    students: groupedByBranch[branch]
  }));
